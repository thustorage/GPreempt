...
void semi_discrete_step( real *state_init , real *state_forcing , real *state_out , real dt , int dir , real *flux, GPUstream stream, int* preempted, int* pStopIndex, int* executed, int kernel_id, int &step_remain);
__device__ void compute_tendencies_x__blp(dim3 taskIdx,  real *state , real *flux , real dt);
__global__ void compute_tendencies_x( real *state , real *flux , real dt, int* preempted, int* pStopIndex, int total_block, int* executed, dim3 shape, int kernel_id);
__device__ void compute_tendencies_z__blp(dim3 taskIdx,  real *state , real *flux , real dt);
__global__ void compute_tendencies_z( real *state , real *flux , real dt, int* preempted, int* pStopIndex, int total_block, int* executed, dim3 shape, int kernel_id);
__device__ void set_halo_values_x__blp(dim3 taskIdx,  real *state );
__global__ void set_halo_values_x( real *state , int* preempted, int* pStopIndex, int total_block, int* executed, dim3 shape, int kernel_id);
__device__ void set_halo_values_z__blp(dim3 taskIdx,  real *state );
__global__ void set_halo_values_z( real *state , int* preempted, int* pStopIndex, int total_block, int* executed, dim3 shape, int kernel_id);
...
void perform_timestep(GPUstream stream, int* preempted, int* pStopIndex, int* executed, int start_from) {
    ...
    int step_count = start_from / 3;
    int step_remain = start_from % 3;
    if(step_count <= 0) semi_discrete_step( state , state     , state_tmp , dt / 3 , DIR_X , flux, stream, preempted, pStopIndex, executed + 0 * 3, 0, step_remain);
    if(step_count <= 1) semi_discrete_step( state , state_tmp , state_tmp , dt / 2 , DIR_X , flux, stream, preempted, pStopIndex, executed + 1 * 3, 1, step_remain);
    if(step_count <= 2) semi_discrete_step( state , state_tmp , state     , dt / 1 , DIR_X , flux, stream, preempted, pStopIndex, executed + 2 * 3, 2, step_remain);

    //z-direction second
    if(step_count <= 3) semi_discrete_step( state , state     , state_tmp , dt / 3 , DIR_Z , flux, stream, preempted, pStopIndex, executed + 3 * 3, 3, step_remain);
    if(step_count <= 4) semi_discrete_step( state , state_tmp , state_tmp , dt / 2 , DIR_Z , flux, stream, preempted, pStopIndex, executed + 4 * 3, 4, step_remain);
    if(step_count <= 5) semi_discrete_step( state , state_tmp , state     , dt / 1 , DIR_Z , flux, stream, preempted, pStopIndex, executed + 5 * 3, 5, step_remain);

    if(step_count <= 6) semi_discrete_step( state , state     , state_tmp , dt / 3 , DIR_Z , flux, stream, preempted, pStopIndex, executed + 6 * 3, 6, step_remain);
    if(step_count <= 7) semi_discrete_step( state , state_tmp , state_tmp , dt / 2 , DIR_Z , flux, stream, preempted, pStopIndex, executed + 7 * 3, 7, step_remain);
    if(step_count <= 8) semi_discrete_step( state , state_tmp , state     , dt / 1 , DIR_Z , flux, stream, preempted, pStopIndex, executed + 8 * 3, 8, step_remain);

    //x-direction first
    if(step_count <= 9)  semi_discrete_step( state , state     , state_tmp , dt / 3 , DIR_X , flux, stream, preempted, pStopIndex, executed + 9  * 3, 9 , step_remain);
    if(step_count <= 10) semi_discrete_step( state , state_tmp , state_tmp , dt / 2 , DIR_X , flux, stream, preempted, pStopIndex, executed + 10 * 3, 10, step_remain);
    if(step_count <= 11) semi_discrete_step( state , state_tmp , state     , dt / 1 , DIR_X , flux, stream, preempted, pStopIndex, executed + 11 * 3, 11, step_remain);

}

__device__ void compute_tendencies_x_r__blp(dim3 taskIdx,  real *state , real *flux , real dt, real * state_init,real *state_out);
__global__ void compute_tendencies_x_r( real *state , real *flux , real dt, real * state_init,real *state_out, int* preempted, int* pStopIndex, int total_block, int* executed, dim3 shape, int kernel_id);
__device__ void compute_tendencies_z_r__blp(dim3 taskIdx,  real *state , real *flux , real dt, real * state_init,real *state_out);
__global__ void compute_tendencies_z_r( real *state , real *flux , real dt, real * state_init,real *state_out, int* preempted, int* pStopIndex, int total_block, int* executed, dim3 shape, int kernel_id);
...
void semi_discrete_step( real *state_init , real *state_forcing , real *state_out , real dt , int dir , real *flux, GPUstream stream, int* preempted, int* pStopIndex, int* executed, int kernel_id, int &step_remain) {
  ...
  if (dir == DIR_X) {
    if(step_remain <= 0) {
      int grid_size = (nz * NUM_VARS + dim - 1) / dim;
      dim3 shape(grid_size, 1, 1);
      static int gridsize = -1, blocksize = -1;
      if(gridsize == -1 && blocksize == -1)
        GPUOccupancyMaxPotentialBlockSizeWithFlags(&gridsize, &blocksize, set_halo_values_x, 0, dim, 0);
      int final_gridSize = std::min(grid_size, gridsize);
      set_halo_values_x<<<final_gridSize, dim, 0, stream>>>(state_forcing, preempted, pStopIndex, grid_size, executed, shape, kernel_id * 3 + 0);
    }
    if(step_remain <= 1) {
      int grid_size = (nz * (nx + 1) + dim - 1) / dim;
      dim3 shape(grid_size, 1, 1);
      static int gridsize = -1, blocksize = -1;
      if(gridsize == -1 && blocksize == -1)
        GPUOccupancyMaxPotentialBlockSizeWithFlags(&gridsize, &blocksize, set_halo_values_x, 0, dim, 0);
      int final_gridSize = std::min(grid_size, gridsize);
      compute_tendencies_x<<<final_gridSize, dim, 0, stream>>>(state_forcing,flux,dt, preempted, pStopIndex, grid_size, executed + 1, shape, kernel_id * 3 + 1);
    }
    if(step_remain <= 2) {
      int grid_size = (nz * nx * NUM_VARS + dim - 1) / dim;
      dim3 shape(grid_size, 1, 1);
      static int gridsize = -1, blocksize = -1;
      if(gridsize == -1 && blocksize == -1)
        GPUOccupancyMaxPotentialBlockSizeWithFlags(&gridsize, &blocksize, set_halo_values_x, 0, dim, 0);
      int final_gridSize = std::min(grid_size, gridsize);
      compute_tendencies_x_r<<<final_gridSize, dim, 0, stream>>>(state_forcing,flux,dt, state_init,state_out, preempted, pStopIndex, grid_size, executed + 2, shape, kernel_id * 3 + 2);
    }
  } else if (dir == DIR_Z) {
    if(step_remain <= 0) { 
      int grid_size = (nx+2*hs * NUM_VARS + dim - 1) / dim;
      dim3 shape(grid_size, 1, 1);
      static int gridsize = -1, blocksize = -1;
      if(gridsize == -1 && blocksize == -1)
        GPUOccupancyMaxPotentialBlockSizeWithFlags(&gridsize, &blocksize, set_halo_values_x, 0, dim, 0);
      int final_gridSize = std::min(grid_size, gridsize);
      set_halo_values_z<<<final_gridSize, dim, 0, stream>>>(state_forcing, preempted, pStopIndex, grid_size, executed, shape, kernel_id * 3 + 0);
    }
    if(step_remain <= 1) { 
      int grid_size = (nx * (nz + 1) + dim - 1) / dim;
      dim3 shape(grid_size, 1, 1);
      static int gridsize = -1, blocksize = -1;
      if(gridsize == -1 && blocksize == -1)
        GPUOccupancyMaxPotentialBlockSizeWithFlags(&gridsize, &blocksize, set_halo_values_x, 0, dim, 0);
      int final_gridSize = std::min(grid_size, gridsize);
      compute_tendencies_z<<<final_gridSize, dim, 0, stream>>>(state_forcing,flux,dt, preempted, pStopIndex, grid_size, executed + 1, shape, kernel_id * 3 + 1);
    }
    if(step_remain <= 2) { 
      int grid_size = (nx * nz * NUM_VARS + dim - 1) / dim;
      dim3 shape(grid_size, 1, 1);
      static int gridsize = -1, blocksize = -1;
      if(gridsize == -1 && blocksize == -1)
        GPUOccupancyMaxPotentialBlockSizeWithFlags(&gridsize, &blocksize, set_halo_values_x, 0, dim, 0);
      int final_gridSize = std::min(grid_size, gridsize);
      compute_tendencies_z_r<<<final_gridSize, dim, 0, stream>>>(state_forcing,flux,dt, state_init,state_out, preempted, pStopIndex, grid_size, executed + 2, shape, kernel_id * 3 + 2);
    }
  }
  step_remain = 0;
}
...
__global__ void compute_tendencies_x( real *state , real *flux , real dt, int* preempted, int* pStopIndex, int total_block, int* executed, dim3 shape, int kernel_id)
{
    while (true) {
        ...
        taskIdx.y = y;
        taskIdx.z = z;
        compute_tendencies_x__blp(taskIdx, state, flux, dt);
    }
}
__device__ void compute_tendencies_x__blp(dim3 taskIdx,  real *state , real *flux , real dt){
  ...
  int id = taskIdx.x * blockDim.x + threadIdx.x;
  int k = id / (nx + 1);
  int i = id % (nx + 1);
...
__global__ void compute_tendencies_x_r( real *state , real *flux , real dt, real * state_init,real *state_out, int* preempted, int* pStopIndex, int total_block, int* executed, dim3 shape, int kernel_id)
{
    while (true) {
        ...
        taskIdx.y = y;
        taskIdx.z = z;
        compute_tendencies_x_r__blp(taskIdx, state, flux, dt, state_init, state_out);
    }
}
__device__ void compute_tendencies_x_r__blp(dim3 taskIdx,  real *state , real *flux , real dt, real * state_init,real *state_out){
  int id = taskIdx.x * blockDim.x + threadIdx.x;
...
__global__ void compute_tendencies_z( real *state , real *flux , real dt, int* preempted, int* pStopIndex, int total_block, int* executed, dim3 shape, int kernel_id)
{
    while (true) {
        ...
        taskIdx.y = y;
        taskIdx.z = z;
        compute_tendencies_z__blp(taskIdx, state, flux, dt);
    }
}
__device__ void compute_tendencies_z__blp(dim3 taskIdx,  real *state , real *flux , real dt){
  int id = taskIdx.x * blockDim.x + threadIdx.x;
...
__global__ void compute_tendencies_z_r( real *state , real *flux , real dt, real * state_init,real *state_out, int* preempted, int* pStopIndex, int total_block, int* executed, dim3 shape, int kernel_id)
{
    while (true) {
        ...
        taskIdx.y = y;
        taskIdx.z = z;
        compute_tendencies_z_r__blp(taskIdx, state, flux, dt, state_init, state_out);
    }
}
__device__ void compute_tendencies_z_r__blp(dim3 taskIdx,  real *state , real *flux , real dt, real * state_init,real *state_out){
  int id = taskIdx.x * blockDim.x + threadIdx.x;
...
__global__ void set_halo_values_x( real *state , int* preempted, int* pStopIndex, int total_block, int* executed, dim3 shape, int kernel_id)
{
    while (true) {
        ...
        taskIdx.y = y;
        taskIdx.z = z;
        set_halo_values_x__blp(taskIdx, state);
    }
}
__device__ void set_halo_values_x__blp(dim3 taskIdx,  real *state ){
  int id = taskIdx.x * blockDim.x + threadIdx.x;
...
__global__ void set_halo_values_z( real *state , int* preempted, int* pStopIndex, int total_block, int* executed, dim3 shape, int kernel_id)
{
    while (true) {
        ...
        taskIdx.y = y;
        taskIdx.z = z;
        set_halo_values_z__blp(taskIdx, state);
    }
}
__device__ void set_halo_values_z__blp(dim3 taskIdx,  real *state ){
  int id = taskIdx.x * blockDim.x + threadIdx.x;
