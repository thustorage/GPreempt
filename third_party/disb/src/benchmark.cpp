...
std::atomic<int> initializedTaskCount(0);
int taskCount;

void BenchmarkTask::runBenchmark(system_clock::time_point beginTime,
...
void BenchmarkSuite::run(void loadCoordinator(const std::vector<LoadInfo> &loadInfos))
{
    ...
    std::thread daemon;
    bool daemonThreadCreated = false;
    if(daemonThread != nullptr) {
        daemon = std::thread(daemonThread, std::ref(stopFlag), tasks.size());
        daemonThreadCreated = true;
    }
